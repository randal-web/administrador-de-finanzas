...
import { CreditCard, Plus, Trash2, DollarSign, TrendingDown } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-0 mb-8">
        <div>
          ...
          <p className="text-sm text-slate-400 dark:text-neutral-500 font-medium mt-2 ml-1">
            Total acumulado: <span className="text-rose-500 font-bold">${totalDebt.toFixed(2)}</span>
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {debts.length === 0 ? (
          <div className="col-span-full flex flex-col items-center justify-center py-12 text-slate-400 dark:text-neutral-600">
            ...
            <p className="font-medium">¡Libre de deudas!</p>
          </div>
        ) : (
          debts.map(debt => (
